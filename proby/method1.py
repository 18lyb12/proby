...
# model
model_1_dir = os.path.join(current_folder_path, 'models/model_1')
...
# output
output_data_folder = os.path.join(current_folder_path, "data/output1")
...
def model_1(metadata):
    ...
    preds_1 = grouped_df[((grouped_df['true_category'].isna()) & (grouped_df['high_pred_score'] == 1)) | (grouped_df['true_category'] == 1)]
...
def generate_comprehensive_prediction(metadata):
    ...
    grouped_model_1_preds_df["high_pred_score"] = grouped_model_1_preds_df['new_category'].apply(lambda x: 1 if x >= threshold else 0)
    ...
    grouped_model_1_preds_df[['activity_score', 'model_1_comments']] = grouped_model_1_preds_df.apply(write_model_pred, axis=1)
    ...
    grouped_model_1_preds_report_df = pd.merge(grouped_model_1_preds_df, reported_smiles_signal_df, on='smiles', how='left')
        ...
        if file_name.endswith('.xlsx'):
            ...
            with pd.ExcelWriter(output_full_path) as writer:
                for i, solvent in enumerate(common_solvents):
                    ...
                    merged_preds_report_1 = pd.merge(df, grouped_model_1_preds_report_df, left_on=column_name, right_on='smiles_', how='left')

                    merged_preds_2 = pd.merge(merged_preds_report_1, model_2_preds_df[model_2_preds_df['Solvent'] == solvent], left_on='smiles_', right_on='Smiles', how='left')
...
def generate_report(metadata):
    ...
    for file_name in os.listdir(comprehensive_folder):
        ...
        if file_name.endswith('.xlsx'):
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                model_1_not_reported_positive_df = model_1_not_reported_df[model_1_not_reported_df["high_pred_score"] == 1]
                model_1_not_reported_negative_df = model_1_not_reported_df[model_1_not_reported_df["high_pred_score"] == 0]
                ...
                print(f"\t\t {len(model_1_reported_smiles)} are reported. "
                      f"{len(model_1_reported_positive_smiles)} positive, "
                      f"{len(model_1_reported_negative_smiles)} negative")

                print(f"\t\t {len(model_1_not_reported_smiles)} are not reported. "
                ...
                assert len(model_2_candidates_pairs) == len(model_1_reported_positive_smiles) + len(model_1_not_reported_positive_smiles)
        ...
        print(f"\t {len(total_model_1_reported_smiles)} are reported. "
              f"{len(total_model_1_reported_positive_smiles)} positive, "
              f"{len(total_model_1_reported_negative_smiles)} negative")

        print(f"\t {len(total_model_1_not_reported_smiles)} are not reported. "
    ...
    print(f" {len(all_model_1_reported_smiles)} are reported. "
          f"{len(all_model_1_reported_positive_smiles)} positive, "
          f"{len(all_model_1_reported_negative_smiles)} negative")

    print(f" {len(all_model_1_not_reported_smiles)} are not reported. "
...
def predict_model_1(test_path, features_path, preds_path):
    ...
    t1 = time.time()
    shared_logger.log(f"model 1 predicting time: {t1 - t0} s")
...
def predict_model_2(test_path, scaled_preds_path, preds_path):
    ...
    preds = chemprop.train.make_predictions(args=args)
    t1 = time.time()
    shared_logger.log(f"model 2 predicting time: {t1 - t0} s")
