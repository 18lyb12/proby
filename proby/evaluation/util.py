...
def classification_evaluation_summary(y_true, y_pred_prob, roc_fig_path="", pr_fig_path=""):
    ...
    # Bootstrap to compute the 95% confidence interval
    def bootstrap_auc(y_true_, y_pred_, n_bootstraps=1000, seed=42):
        rng = np.random.RandomState(seed)
        bootstrapped_aucs = []

        for iteration in range(n_bootstraps):
            # Sampling with replacement
            indices = rng.choice(len(y_true_), len(y_true_), replace=True)

            # Ensure that the sampled data contains at least two classes
            if len(np.unique(y_true_[indices])) < 2:
                continue

            fpr_boot, tpr_boot, _ = roc_curve(y_true_[indices], y_pred_[indices])
            bootstrapped_aucs.append(auc(fpr_boot, tpr_boot))

        # Compute the 95% confidence interval (2.5% - 97.5%)
        lower = np.percentile(bootstrapped_aucs, 2.5)
        upper = np.percentile(bootstrapped_aucs, 97.5)
        return lower, upper

    ci_lower, ci_upper = bootstrap_auc(y_true, y_pred_prob)

    print(f"ROC AUC: {roc_auc:.2f}, 95% CI = {ci_lower:.3f} - {ci_upper:.3f}")
